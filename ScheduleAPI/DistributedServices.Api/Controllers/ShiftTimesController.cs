...
{
    public class ShiftTimesController : ApiController
    {
        private readonly IShiftTimeRepository _repository;

        public ShiftTimesController()
        {
            _repository = IoCFactory.Resolve<IShiftTimeRepository>();
        }
        ...
        public HttpResponseMessage Post([FromBody]ShiftTime item, [FromUri]string clientToken)
        {
            if (item == null)
                return Request.CreateResponse(HttpStatusCode.OK, new ShiftTime());
        ...
        public HttpResponseMessage Put([FromUri]int id, [FromBody]ShiftTime item, [FromUri]string clientToken)
        {
            if (item == null)
                return Request.CreateResponse(HttpStatusCode.OK, new ShiftTime());
